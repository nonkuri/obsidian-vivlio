...
/** One document of the package, as the manifest and the nav need to see it. */
interface EpubDocument {
  id: string;
  href: string;
  chapter: Chapter;
  /** Whether the document carries MathML, for the manifest property. */
  hasMathml: boolean;
}
...
export async function buildEpub(
  ...
  for (const chapter of chapters) {
    ...
    const xhtml = toXhtml(context, html);
    oebps.file(href, xhtml);
    // A document carrying MathML has to say so in the manifest: a reading
    // system decides from the property whether it can render the document at
    // all, and an EPUB that keeps it to itself does not validate.
    documents.push({ id, href, chapter, hasMathml: /<math[\s>]/.test(xhtml) });
  }
    ...
    ...documents.map(
      (entry) =>
        `    <item id="${entry.id}" href="${entry.href}" media-type="application/xhtml+xml"${
          entry.hasMathml ? ' properties="mathml"' : ""
        }/>`,
    ),
    // Every href is generated by assetFileName and needs no escaping today.
    // It is escaped anyway: this document is assembled as text, and a package
    // document that is not well-formed loses the book, not the one file.
    ...assets.map(
      (asset) =>
        `    <item id="${asset.id}" href="${escapeXml(asset.href)}" media-type="${
          escapeXml(asset.mime)
