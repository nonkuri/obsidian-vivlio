...
[前: テーマ別の原稿](05a-theme-writing.md) / [マニュアル目次](00_vivlio_plugin_manual.md) / [次: 書き出しとトラブルシューティング](07-export-and-troubleshooting.md)
...
- 同梱テーマ名。通常は `novel`
- **Vault 内にある一枚の CSS ファイルへのパス**（`./`・`../` は指定元の YAML／ノート基準、それ以外は Vault ルート基準）
...
| `vivlio:paper` | academic を土台にした横組みの論文・レポート |
...
同梱の `manual` を使う原稿の書き方は、[テーマ別の原稿：manual](05a-theme-writing.md#manual操作ガイド技術書) に移しました。図表の表題・自動採番・原稿間参照は [図表番号と参照](05a-theme-writing.md#manualpaper共通図表番号と参照) を参照してください。

## 論文・レポートを組む

同梱の `paper` の設定・見出しの採番・節参照・付録の指定は、[テーマ別の原稿：paper](05a-theme-writing.md#paper論文レポート) を参照してください。

## 配布CSSを使う

自分でCSSを書かずに見た目を変更したい場合は、[CSSカスタマイズ集](../sample/README.md#cssカスタマイズ)から選びます。原稿や画像の構成もまとめて試したい場合は、[完成サンプル](../sample/README.md#完成サンプル)を使ってください。

1. 各作例の説明で、対応するベーステーマ・組方向・段数・バージョン・必要な原稿記法を確認します。
2. CSSのリンク先で **Raw** を開き、Vault内へ `.css` ファイルとして保存します。例：`装丁/novel-title-page.css`。
3. 本の既存の `vivlio.yaml` で、`theme` を次のように指定します。同じキーがある場合は追記せず書き換えます。

   ```yaml
   theme: 装丁/novel-title-page.css
   ```

4. 作例で指定されている設定も反映し、プレビューの **再ビルド** を押します。
5. 変更した箇所と前後のページを確認します。PDF・EPUBの両方を使う場合は、それぞれ書き出して確認してください。

この例の `装丁/novel-title-page.css` は **Vault ルート基準**です。本と一緒に CSS を移動・配布する場合は、YAML と同じフォルダに置いて `theme: ./novel-title-page.css` と書く方法を推奨します。`../` も使えます。frontmatter ではそのノートのフォルダを基準にします。テーマを選んだだけでは、判型・組方向・段数の明示設定は変わりません。本のYAMLに `theme` がある場合は、ツールバーや設定タブの既定テーマより優先されます。

このCSSはVivlioの書籍用です。Obsidianの外観設定の「CSSスニペット」では設定しません。「CSSのみ」の配布でもYAMLの設定が必要になる場合がありますが、設定例は各作例のREADMEからコピーできます。

### 図版やコラムを含む縦書き段組み

[図版付きの縦書き段組みサンプル](../sample/css/vertical-feature/README.md)には、独立CSSに加えて、そのまま試せるMarkdown原稿・YAML・SVGを同梱しています。`vertical-feature` フォルダをVault内の任意の場所へ置き、内側の `book` フォルダを組版対象にします。説明用READMEは本文フォルダの外にあります。

この作例では、原稿先頭の図版は固定高の枠へトリミングし、本文途中の画像は切り抜かず一段内へ収めます。`> [!note]` や `> [!tip]` は囲みコラムになります。先頭ページの本文は三段、続きは四段で、柱・ノンブルは左右の小口側です。見開き表示は[プレビュー設定](03-settings.md#プレビュー)で選び、一つ先へ進んで2・3ページを確認します。画像寸法や段数の変更、複数原稿の扱いはサンプルのREADMEを参照してください。

### 縦書き・横書きの印刷用マガジン

[縦書き版](../sample/css/vertical-magazine/README.md)と[横書き版](../sample/css/horizontal-magazine/README.md)は、A4・3mm塗り足し・トンボ付きの独立CSSです。内蔵テーマのimportは不要で、原稿・画像・設定を含むZIPから試せます。導入方法は[印刷用マガジンを試す](02-first-book.md#印刷用マガジンを試す)を参照してください。

どちらも、先頭画像・H1の題名・著者段落・リード段落・H2以下の本文、の順に原稿を書きます。縦書きは三段を基本に冒頭図版で一段を使い、横書きは二段の上に図版・見出し・リードを配置します。コールアウトと本文途中の図版にも対応します。

塗り足しはYAMLの `bleed: 3mm` と、画像そのものを仕上がり線の外へ伸ばすCSSの両方で実現しています。紙面サイズや余白を変える場合は、図版の幅・高さ・負のマージンも合わせて見直してください。本文画像は紙面内、表紙は四辺、冒頭画像は上・左右の三辺という違いがあります。設定と差し替え手順は各READMEにまとめています。
...
`vivlio.yaml` の `theme` にパスを書きます。次は Vault 共通の「装丁」フォルダを使う例です。本に同梱する CSS なら `./遠雷.css` などの相対指定を推奨します。
...
各 CSS から相対パスで参照した画像やフォントも、その CSS が置かれた場所を基準に解決されます。対象ファイルはテーマ用アセットとして書き出しへ収録されるため、import した CSS の背景画像も EPUB に含まれます。ローカルフォントは **EPUB にフォントを埋め込む**がオンの場合だけ同梱します（既定はオフ）。`data:`、`http:`、`https:`、`#fragment` の URL はそのまま残ります。
...
複数原稿の結合、図の浮動配置、`vivlio-paper-role` の説明は [テーマ別の原稿：paper](05a-theme-writing.md#paper論文レポート) にまとめました。自作 CSS で `vivlio:paper` を import するだけでは、`theme: paper` 専用の組み立て・採番処理は有効になりません。
