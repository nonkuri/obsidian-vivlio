...
`theme` は **Vaultのルートからのパス**です。YAMLと同じフォルダを基準にしたパスではありません。テーマを選んだだけでは、判型・組方向・段数の明示設定は変わりません。本のYAMLに `theme` がある場合は、ツールバーや設定タブの既定テーマより優先されます。

このCSSはVivlioの書籍用です。Obsidianの外観設定の「CSSスニペット」では設定しません。「CSSのみ」の配布でもYAMLの設定が必要になる場合がありますが、設定例は各作例のREADMEからコピーできます。

### 図版やコラムを含む縦書き段組み

[図版付きの縦書き段組みサンプル](../sample/css/vertical-feature/README.md)には、独立CSSに加えて、そのまま試せるMarkdown原稿・YAML・SVGを同梱しています。`vertical-feature` フォルダをVault直下へ置き、内側の `book` フォルダを組版対象にします。説明用READMEは本文フォルダの外にあります。
