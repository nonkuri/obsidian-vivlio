...
テーマ選択欄に標準表示される同梱テーマは、プラグインの構造と突き合わせて確認済みの `novel`（縦組み）、`novel-2col`（縦組み二段組）、`english-novel`（英語小説の横組み）、`manual`（横組み）、`paper`（論文・レポート）です。ほかのテーマは `vivlio:` import から利用できますが、扉・目次・奥付・柱・ノンブル・縦横組みを必ず確認してください。

## マニュアル・操作ガイドを組む

`manual` は手順を追いながら読む横組みの資料向けです。見出しの罫線、注意・ヒントの枠、明るい背景のコード、キーボード表記、長い表の列見出しの繰り返しを備えています。
...
本文の既定は字下げなしです。文章中心のハンドブックでは `paragraphIndent: 1em` で従来の字下げに戻せます。注意書きには `> [!warning] 注意：原稿を保存する`、補足には `> [!note] 補足：…`、ヒントには `> [!tip] ヒント：…` を使います。色だけで区別しなくて済むよう、タイトルにも種類を書いてください。手順は通常の番号付きリスト、キーは `<kbd>Enter</kbd>` と書けます。

キャプションを持つ本文の図・表は、原稿をまたいで「図1」「表1」からそれぞれ通し採番します。図は `figure` と `figcaption`、表は `table` と `caption`（または表を囲む `figure` と `figcaption`）で記述します。通常のMarkdown表に表題は自動追加しません。

```html
<figure id="fig-start">
  <img src="figures/start.svg" alt="操作の流れ">
  <figcaption>開始から確認まで</figcaption>
</figure>

<p><a href="#fig-start" data-ref="fig"></a>に操作の流れを示します。</p>
```
...
[マニュアルサンプル](../sample/manual/index.md) は4原稿、模式図2点、設定表と36行のチェックリストを含みます。追加CSSなしで、手順・注意書き・長いコード・改ページを確認できます。

## 論文・レポートを組む

新規作成ウィザードで **論文・レポート（A4・横組み）** を選ぶと、`paper` テーマ、A4、横組み、一段組が設定されます。既存の本では、テーマ選択だけでは判型・組方向は変わらないため、次のように設定します。
...
`paper` は明朝系の本文、ゴシック系の見出し、下中央のノンブルを持ちます。タイトルとして取り除かれた見出しを除き、本文で最上位の見出しを `1`、その下を `1.1` のように自動採番します。番号は複数の原稿ノートを通して続きます。見出しに番号を手書きすると重複します。

図表番号・参照には VFM の図表マークアップを使います。通常の Markdown 表には自動で表題は付きません。[論文サンプル](../sample/paper/index.md) と [設定例](../sample/paper/vivlio.yaml) は、本文7ファイル、図5点、表6点、数式・脚注・参考文献を含みます。

`academic` は上流の元テーマ、`paper` は Vivlio 向けの調整版です。既存の `theme: academic` と `@import url("vivlio:academic");` の意味は変わりません。投稿先固有の書式は必要に応じて自作 CSS で調整してください。

複数ファイルでの動作確認には [paper サンプル](../sample/paper/index.md) を使えます。合成信号の計算実験を題材に、図表・本文の自然な改ページ、ファイル間参照、48行の長表の分割を確認できます。追加CSSや手入力の番号を使わず、標準テーマで自動採番します。フォルダ全体を Vault にコピーし、同梱の `vivlio.yaml` からプレビューしてください。
...
`theme: paper` は本文原稿を一つの組版文書にまとめ、ファイル境界では改ページしません。
この処理はVivlio 0.14.0以降で使えます。CSSで `vivlio:paper` をimportするだけでは有効になりません。
明示的な改ページ指定は維持されます。編集用のノートは分割したままです。
章・節、図表は出現順に自動採番され、目次と `data-ref="sec"` / `fig` / `tbl` の参照も更新されます。
番号は出力HTMLに含まれるため、EPUBでも同じ番号になります。

図はキャプションと一緒にページ上部へ浮動配置し、原稿中の図の位置に空白が残る場合は後続の本文を流します。
図の順序と指定寸法・縦横比を保ち、参考文献・付録の開始時には、それ以前の未配置の図を出し切ります。
本文中では「下の図」ではなく図番号で参照してください。図の数や大きさによっては図だけのページになる場合があります。
長い表は浮動配置せず、列見出しを繰り返しながらページをまたぎます。
ページフロートに対応しないEPUB読書アプリでは、図は原稿順の通常配置になります。

番号の役割は各原稿のフロントマターで指定します（本全体のYAML設定ではありません）。
...
| `abstract` | 要旨内の見出しを番号なしにする |
| `references` | 文献内の見出しを番号なしにし、URLの脚注への重複出力を止める |
| `appendix` | 付録A、A.1、付録B…と自動採番する |
| `unnumbered` | 見出しを番号なしにする |

「Vivlioプロパティ」の追加・編集でも「論文内での役割」を選べます。
見出しやキャプションへ番号を手入力する必要はありません。
本文中の図表・節番号は、IDを参照するリンクで指定してください。
原稿をまたぐ同名IDは出力時に区別され、脚注などのローカルリンクも維持されます。
