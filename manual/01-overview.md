...
既定の `novel` は Vivlio 用に調整した縦組み小説テーマ、`novel-2col` はそれを二段組にしたもので、同人誌の B6・A5 や新書に向きます。`manual` は同じく Vivlio 用の横組みテーマで、マニュアルや技術書に向きます。このマニュアル自体が `manual` で組まれています。Vault 内の任意の `.css` をテーマにでき、`@import url("vivlio:novel");` から少しずつ変更できます。

プラグインには Vivliostyle の `base`、`bunko`、`techbook`、`academic` も同梱されていますが、テーマ選択欄に標準表示されるのは動作確認済みの `novel`、`novel-2col`、`manual` だけです。ほかの同梱テーマは自作 CSS から `vivlio:base` などとして読み込めます。
