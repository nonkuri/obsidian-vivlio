...
複数の出力設定を使い分けたい場合は、`print.yaml`、`ebook.yaml` など任意の `.yaml` ファイルを同じフォルダーに置けます。使いたい YAML を開いた状態で **Vivlio: PDF に書き出す**または **Vivlio: EPUB に書き出す**を実行すると、そのフォルダー全体を一冊として、選択した設定で書き出します。ファイルエクスプローラーで YAML を右クリックして **この本を PDF に書き出す**または **この本を EPUB に書き出す**こともできます。

Markdown ノートやフォルダーからプレビュー・書き出しを始めた場合は、従来どおり同じフォルダーの `vivlio.yaml` が自動的に使われます。別名の設定を使うときだけ、その YAML を直接開くか右クリックしてください。
...
- `.yaml` が2つ以上なら、どの設定を使うか暗黙に決めず、1つずつ選択するよう表示します。同じフォルダー内の複数設定も別々に数えます。
