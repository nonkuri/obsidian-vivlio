...
**塗り足しまで自動的に埋まるのは表紙だけです。** 表紙は版面の内側ではなく紙そのものとして組まれ、画像が紙いっぱいに広がるので、塗り足しのための指定は要りません。いっぽう**本文の画像は版面に収められる**ので、そこで止まります。プラグインも同梱テーマも**ページ背景を塗らない**ので、ほかに紙の端まで届くものもありません。

断ち切りの挿絵や地色のあるページが要るときは、本の `css` で自分で書いてください。このとき `body` の箱は版面なので、`body { background: … }` では版面の内側しか塗られません。紙全体を塗るには `@page` に書きます。
