...
| キー | 値 | 説明 |
|---|---|---|
| `pageNumbering` | `roman-then-arabic` / `continuous` / `none` | ノンブル方式。`roman-then-arabic` では前付けが `i, ii, iii…`、本文の一章目から `1` に戻ります。目次のページ番号も同じ書式で刷られ、前付けの項目はローマ数字になります。`none` では紙面にノンブルが出ないので、目次もページ番号を載せません。 |
...
**章や献辞を「必ず左ページから」始める**には `startSide: left` を指定します。和書は右綴じなので、奇数ページが左にきます。本文がそこまで届かなければ**白丁（空白ページ）が 1 枚入り**、その 1 枚は**ノンブルを刷りませんが、ページ数には数えます**（数えないと以降のノンブルがずれるためです）。表紙は先頭の一葉なので対象外です。
