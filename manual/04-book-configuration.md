...
| キー | 値 | 説明 |
|---|---|---|
| `pageNumbering` | `continuous` / `roman-then-arabic` / `none` | ノンブル方式。初期値 `continuous` は表紙と表紙裏を除き、扉・目次・白丁・まえがき・本文・後付けを算用数字で通します。`roman-then-arabic` では前付けをローマ数字で数え、本文の一章目から算用数字を数え直します。目次のページ番号も紙面と同じ書式になります。`none` では紙面にも目次にも番号を載せません。 |
