# Vivlio

0.18.0追加：エディタとプレビューの段落単位の双方向連動。プレビューの文章をクリックすると原稿へ移動し、開いていない原稿は画面を分割せず新規タブで開きます。

[English](README.md) | **日本語**

Obsidian のノートを [Vivliostyle](https://vivliostyle.org/) で組版します。CSS 組版（paged media）による縦書き、ルビ、傍点に対応し、プレビューを見ながら PDF と EPUB に書き出せます。

### 30秒で見るVivlio

日本語・欧文の小説、短歌、図表のある論文、マニュアル、縦書き・横書きの雑誌まで。さまざまな紙面と、トンボ入り原稿の作例を30秒で紹介します（音声なし）。

https://github.com/user-attachments/assets/164de8f8-75cb-405d-9647-388da9f9522d

デスクトップ専用です（`isDesktopOnly: true`）。初回起動時に何もダウンロードしません。組版エンジンはプラグインに同梱されていて、PDF は Obsidian がすでに動かしている Chromium が刷ります。

仕様は [docs/SPEC.md](docs/SPEC.md) にあります。設定タブから `vivlio.yaml`、原稿の書き方、テーマの自作、トラブルシューティングまでを扱った詳細マニュアルを [manual/](manual/00_vivlio_plugin_manual.md) にまとめています。

![左に Obsidian のノート、右に Vivlio のプレビュー。エディタに素のまま置かれた vivlio-* フロントマターと ｜遠雷《えんらい》・《《…》》 の記法が、隣のペインでは縦組みの本文になっている。](docs/images/obsidian.png)

*左がノート、右がページ。プレビューは PDF と同じエンジン、同じスタイルシートを使います。*

プレビューだけでできないことが 1 つあります。目次のページ番号は、全ページが組まれるまで `??` と表示されます。番号は `target-counter` が数えるもので、まだ組まれていないページには数える対象がないためです。PDF の書き出しでは本全体を組んで番号を確定します。リフロー型の EPUB では、目次のページ番号を非表示にします。画面上でも実際の番号を見たい場合は、設定の **全ページを最初に描画する** を有効にしてください。プレビューの表示までは遅くなりますが、最初から正しい番号が出ます。

## できること

プレビューの **カーソル連動：オン／オフ** ボタンは現在の状態を表示し、押すとそのペインの両方向の連動を切り替えます。ペインを開いたときはオンです。エディタでカーソルを動かすと、その段落の先頭があるページへプレビューが移動します。プレビューの文章をクリックすると、元ノートの段落先頭行へカーソルとスクロール位置を移動します。開いているノートは既存タブを使い、開いていないノートは画面を分割せず新規タブで開きます。見出し・箇条書きにも対応します。リンクのクリックや文字の範囲選択は通常どおり使えます。

編集中の原稿とプレビューが一致するまでは位置移動を待ちます。自動更新が無効の場合は、編集後に **再ビルド** を押してください。複数ページにまたがる段落への追従は段落の先頭ページが対象です。対応する原稿の段落がない自動生成部分は、移動の対象になりません。

| | |
|---|---|
| **プレビュー** | 実際のページの組み上がりを表示します。PDF と同じエンジン・同じスタイルシートです。縦書き、ぶら下げ組、和欧間のアキまで含まれます。いずれも Obsidian 標準の PDF 書き出しでは得られません。 |
| **1 つのノートでも、まとめてでも** | ノート 1 つがそのまま 1 冊になります。フォルダを指定すればその直下のノートが、目次ノートを指定すればそれが `[[リンク]]` で並べたノートが、1 冊になります。 |
| **Obsidian の記法** | 埋め込み、ウィキリンク、コールアウト、タスクリスト、タグ、ハイライト。加えて青空文庫・カクヨム式のルビ、傍点、縦中横。 |
| **PDF** | タグ付き、検索可能。しおり、メタデータ、`i, ii, iii, 1, 2 …` のページラベル付き。フォントは Chromium が埋め込んでサブセット化するので、他の環境でも印刷できます。 |
| **EPUB 3** | リフロー型。テーマの CSS、表紙、ランドマークを含みます。 |
| **書き出し前の点検** | 300 dpi を下回る画像、この PC にないフォント、ページと縦横比の合わない表紙を報告します。 |

## サンプル・カスタマイズ

印刷用マガジンの完成サンプル：[縦書き「余白通信」](sample/css/vertical-magazine/README.md) ／ [横書き「FIELD NOTES」](sample/css/horizontal-magazine/README.md)。縦書きは右綴じ・三段（冒頭は図版と本文二段）、横書きは左綴じ・二段と段抜き見出しです。どちらもA4・3mm塗り足し・トンボに対応し、独立CSS、オリジナル原稿2本、SVG4点、4ページの組見本PDF、素材の来歴とライセンスを一式ZIPで配布しています。Vivlio 0.17.3で検証済みのVault内テーマです。

- **自分の原稿の見た目を変えたい** → [CSSカスタマイズ集](sample/README.md#cssカスタマイズ)。章扉や、図版・囲みコラムを含む縦書き段組みのCSSと使い方を掲載しています。
- **原稿と設定をまとめて試したい** → [完成サンプル集](sample/README.md#完成サンプル)。小説・エッセイ・句集／歌集・マニュアル・論文の原稿、設定、組見本を一式で入手できます。

対応バージョンとダウンロードは[サンプル一覧](sample/README.md)、共通の導入手順は[マニュアル](manual/02-first-book.md#サンプル一式で試す)を参照してください。

[図版付きの縦書き段組み](sample/css/vertical-feature/README.md)は、A4の縦四段を基本に、先頭図版・段内画像・コールアウト・見開きの小口側の柱とノンブルを試せます。Markdown原稿、YAML、独立CSS、オリジナルSVGと見開きの組見本を同梱しています。

## インストール

**Obsidian から。** 設定 → コミュニティプラグイン → 閲覧 で *Vivlio* を検索し、インストールして有効化します。

**手動で。** [リリース](https://github.com/nonkuri/obsidian-vivlio/releases) から `main.js`、`manifest.json`、`styles.css` を取得し、`VaultFolder/.obsidian/plugins/vivlio/` に置きます。Obsidian を再読み込みして、コミュニティプラグインから有効化してください。

デスクトップ版の Obsidian 1.8.7 以降が必要です。PDF は Obsidian がすでに動かしている Chromium で刷るため、モバイル版はありません。

## プレビューを表示する

**設定 → Vivlio → プレビュー**で、単ページ・見開き・自動、画面フィット（初期値オン）、固定ズーム（10〜1000%、初期値100%）を設定できます。設定はプレビューを開くとき・再組版時に適用されます。**ビューアでの変更を記憶する**は初期値オンで、表示モードとズーム・フィットの変更を Vault 共通の既定値として保存し、再組版・Obsidian 再起動後も復元します。オフにすると再組版時にプラグイン設定へ戻ります。表示設定は `vivlio.yaml` への書き出し対象外で、PDF・EPUB 出力には影響しません。用紙や余白などは従来の本設定で管理します。詳しくは[設定マニュアル](manual/03-settings.md#プレビュー)を参照してください。

方法は三つ、手近なものをどうぞ。

- **リボン** — 左のリボンにある本のアイコンで、開いているノートを組版します。
- **コマンドパレット** — `Vivlio: プレビューを開く` も同じ動きです。
- **ファイルエクスプローラの右クリックメニュー** — Markdown ノートを右クリック
  すると **Vivlio: プレビュー**。*フォルダ*を右クリックすると **Vivlio: 本と
  してプレビュー**（フォルダ内のすべての `.md` を章の順序で）と、その隣に
  **Vivlio: 本として書き出す** が出ます。

ペインは右側に開き、上部にツールバーが載ります。**再組版**、テーマの選択、
そして表示中のものをそのまま書き出しダイアログに渡す **PDF** / **EPUB** の
ボタンです。ノートを編集すれば組み直しますが、設定タブの
**プレビューを自動更新する** をオフにすれば止まります。その場合はツールバーの
ボタンか `Vivlio: 組版結果を再読み込み` で組み直してください。

## コマンド

| コマンド | 内容 |
|---|---|
| `Vivlio: プレビューを開く` | アクティブなノートをサイドペインで組版 |
| `Vivlio: PDF に書き出す` / `EPUB に書き出す` | 書き出しダイアログ、点検、そしてファイル |
| `Vivlio: このフォルダを本として書き出す` | フォルダ内のすべての `.md` を順に |
| `Vivlio: このノートを目次として本を組む` | ノートの `[[リンク]]` が本文の並び順になる |
| `Vivlio: 本の設定を作成` | `vivlio.yaml` を書くウィザード。全キーを書き出し、既定のままのキーはコメント行にします |
| `Vivlio: このノートに設定を追加` | フラットな `vivlio-*` フロントマターを説明つきで追加・変更・削除 |
| `Vivlio: 設定リファレンスを書き出す` | すべてのキーを既定値とコメント付きで |

ファイルエクスプローラの右クリックメニューからもプレビューと書き出しができます
（「プレビューを表示する」を参照）。

任意の `.yaml`（例: `print.yaml`、`ebook.yaml`）を開いて **PDF に書き出す**または **EPUB に書き出す**を実行すると、そのファイルがあるフォルダ全体を、選択した YAML の設定で一冊として書き出します。ファイルエクスプローラーで YAML を右クリックし、PDF / EPUB を直接選ぶこともできます。通常どおり Markdown やフォルダーから実行した場合は、従来どおり同じフォルダーの `vivlio.yaml` を自動的に使います。

複数選択に `.yaml` が1つだけ含まれる場合は、そのファイルを設定として同じ操作ができます。複数の `.yaml` が含まれる場合は、どの設定を使うか曖昧になるため、一括書き出しはせず「設定 YAML は1つずつ選択してください」と表示します。同じフォルダー内の `print.yaml` と `ebook.yaml` を選んだ場合も同様です。

任意名の YAML を開いて **本の設定を作成**を実行した場合も、その内容を初期値としてウィザードを開き、完了時は選択した同じファイルを更新します。Markdown から実行した場合は `vivlio.yaml` を作成または更新します。

表示言語は既定で Obsidian の設定に追随します（設定タブで固定もできます）。

## 本の設定

3 つの層があり、下の層が上の層を上書きします。

1. **設定タブ** — Vault 全体の既定値。
2. 本の隣に置く **`vivlio.yaml`** — 本の設定を書く本来の場所です。入れ子とコメントが使えます。
3. **ノートのフロントマター** — Obsidian のプロパティエディタで編集できるフラットな `vivlio-*` キーを推奨します。手書きの入れ子形式 `vivlio:` も読み取れます。

単独ノートではそのノート、フォルダや YAML から組む本では目次ノート、リンクから組む本では選択したノートが本全体の設定元になります。通常の章ノートの指定で、その章だけのテーマや組方向を上書きすることはできません。章ごとのスタイルはクラスと CSS で指定します。`vivlio-order`、`vivlio-toc`、`vivlio-paper-role` はノート固有の情報です。

### `.yaml` と `.css` を Obsidian 内で編集する

標準の Obsidian は Markdown ノートを中心にしたエディタで、任意の `.yaml` や `.css` を編集する機能は備えていません。Vivlio は設定の **「.yaml / .css / .epub をファイルエクスプローラーに表示する」** がオンなら、`vivlio.yaml` と自作テーマを表示し、最低限のプレーンテキストエディタで開けるようにします。

構文の色分け、行番号、コードの折りたたみ、検索・置換なども Obsidian 内で使いたい場合は、コミュニティプラグインの [Code Space](https://community.obsidian.md/plugins/code-space) が便利です。この README の作者も利用しています。**設定 → コミュニティプラグイン → 閲覧**で「Code Space」を検索してインストールしてください。`.css`、`.yaml`、`.yml` は標準の管理対象です。

Code Space を入れたのにファイルが Vivlio の簡易エディタで開く場合は、Vivlio の上記設定をオフにして Obsidian を再起動し、Code Space の **Managed extensions** に `css`、`yaml`、`yml` が含まれていることを確認します。1つの拡張子を同時に複数のプラグインが担当できないためです。外部フォルダのマウント機能は、この用途には必要ありません。

```yaml
# vivlio.yaml
title: 吾輩は猫である
author: 夏目漱石

theme: novel              # novel、novel-2col、essay、haiku、tanka、english-novel、manual、paper、または Vault 内の CSS のパス
writingMode: vertical-rl
size: 文庫
charsPerLine: 39
linesPerPage: 15
footnote: gcpm            # ページ下部に

cover: 装丁/表紙.png
# 裏表紙は任意:
# backCover: 装丁/裏表紙.png
# backCoverFit: contain
sections:
  titlePage: auto
  toc: auto
  preface: まえがき.md
  colophon: auto
pageNumbering: continuous # 表紙・裏表紙とそれらの番号対象外の白紙を除く通し番号
startPage: 1             # 通し番号の開始値。0以下は数えるが表示しない
cropMarks: false         # トンボなし
bleed: 3mm               # 塗り足し 3mm。用紙がそのぶん広がります
output: 原稿/出力/猫.pdf
```

`bleed` はトンボの有無にかかわらず指定できます。トンボなしのときは用紙を仕上がり寸法＋塗り足し×2
で書き出すので、日本の印刷会社の言う「トンボなし・塗り足し3mm」がそのまま作れます。版面は仕上がり
から見た位置のまま動きません。

`backCover` で末尾に裏表紙を追加できます。`backCoverFit` は `cover`（全面を埋めて切り抜き、既定値）または `contain`（画像全体を収める）です。PDF・プレビューでは内側を白紙にし、ノンブルの開始値に関係なく最後の物理的な偶数ページへ配置します。裏表紙と追加した白紙にはノンブル・柱を出さず、目次にも載せません。PDF書き出しの **表紙・裏表紙を含める**で両方をまとめて切り替えます。EPUBには白紙なしで画像だけを末尾に追加し、本棚サムネイルは表紙画像を使います。

表紙・裏表紙画像と `coverPage` の地色は、トンボの有無にかかわらず塗り足しの外端まで届きます。
本文で 1 ページの断ち切り画像にするには `![[images/挿絵.png|bleed]]` と書きます。地色のページは
`<div class="vivlio-bleed" style="background: #18202a"></div>` のように、紙面にする要素へ
`vivlio-bleed` クラスを付けます。通常の本文画像は従来どおり版面内に収まります。

```yaml
---
title: 吾輩は猫である
vivlio-theme: manual
vivlio-size: 文庫
---
```

ノート自身の `title` も読むので、1 つのノートを単独で書き出すぶんには `vivlio-title` は要りません。書名とノートのタイトルを分けたいときに `vivlio-title` を書きます（そちらが優先されます）。`Vivlio: このノートに設定を追加` が挿入するのはこの形です。このコマンドが提示するキーはすべて `vivlio-` を付けた名前になるためです。

`Vivlio: 本の設定を作成` は、これらのキーを一つずつ尋ねたうえで**全キーを書き出します**。**既定値を使う**のままにしたキーはコメント行として書かれるので、ファイルはその本に何が指定できるかの一覧を兼ね、しかも本は Vault の既定に追従し続けます。`#` を消せば、その項目だけこの本のものになります。

欧米向けの英語小説は、ウィザードで英語小説のプリセットを選ぶか、次の設定から始められます。

```yaml
lang: en
theme: english-novel
writingMode: horizontal-tb
size: 6x9
sections:
  titlePage: auto
  copyrightPage: auto
  toc: auto
  colophon: off
```

英語本では、文章形式の copyright page を扉の直後に置き、日本語式の奥付 `colophon` は独立した任意の巻末部位として扱います。両方を省略した場合、`lang: en` なら上記の値が既定になります。ウィザードは目次見出しや copyright page の文章を含む `labels:` も選択言語に合わせて書き出すので、テーマを変更せず文言だけ編集できます。

```yaml
# --- 組版 ---
# 判型: 文庫（A6・105x148mm）| 新書 | JIS-B6 | A5 | ...
# size: 文庫
# 行あたりの文字数。空ならテーマが判型と文字サイズから決める
charsPerLine: 39
```

`Vivlio: 設定リファレンスを書き出す` を実行すると、すべてのキーを既定値とコメント付きで、コメントではなく値として列挙した `vivlio.yaml` が得られます。

## 章の順序

1. 目次ノート（`index.md`、フォルダ名と同じ名前のノート、または `vivlio-toc: true` を持つノート）があれば、その `[[リンク]]` の並び順。
2. なければファイル名の自然順。`2.md` は `10.md` より前に来ます。
3. `vivlio-order: 3` はどちらの場合も位置を固定します。

目次ノート自身は、`includeToc: true` としない限り本には含まれません。

`vivlio-order` と `vivlio-toc` は本ではなくノートに属する設定なので、`vivlio.yaml` には書けず、設定リファレンスにも出てきません。`Vivlio: このノートに設定を追加` が両方を提示します。

## 記法

| 書くと | こうなる |
|---|---|
| `《《テキスト》》` | 傍点（カクヨム式）。ゴマ点・丸・三角などから選択でき、任意の記号も入力できます |
| `漢字《かんじ》` | 直前の漢字の連なりにルビ。原稿がほぼこの形で書く省略形（`々〆〇ヵヶ` も漢字として扱います） |
| `｜任意《よみ》` | 任意の親文字にルビ。`｜` が親文字の始まりを示します（半角の `\|` も可） |
| `{漢字\|かんじ}` | ルビ（VFM 本来の記法） |
| `^^1/2^^` | 縦中横。4 文字まで |
| 縦書き中の 1〜2 桁の半角数字 | 自動で正立。2 桁は 1 em に組み合わせ、1 桁は横倒しにせず立てます。前後に数字・英字・`. , : % -` が隣接しないときだけ |
| `==ハイライト==` | 傍点、太字、`<mark>`、または素のテキスト — 選べます |
| `［＃改ページ］`、または `===` だけの行 | 強制改ページ。青空文庫式の注記と、でんでんマークダウン式のイコール 3 つ以上の行、どちらでも書けます。イコールの行の上には空行を空けてください（詰めて書くと Markdown が見出しの下線と読みます） |
| 3 行以上の連続した空行 | 紙面の空き。`n` 行の空行が `n - 2` 行アキになります |
| 行頭の全角スペース | その段落を字下げする指示として読み、文字自体は取り除きます |
| `> [!任意]` | 囲み罫のコールアウト。種別は何でもよく、`callout-<種別>` として残るのでテーマで差をつけられます |
| `![[fig.png\|300]]` | 幅を指定して画像を配置。`300`、`300x200`、`60%`、`80mm`、`300px` |
| `![[fig.png\|bleed]]` | 画像だけの裁ち落としページ。画像を塗り足しの外端まで広げます |
| `![キャプション](fig.png)` | 単独の段落ではキャプション付きの `<figure>`。`![キャプション\|50mm](fig.png)` のように幅も指定でき、サイズ指定はキャプションに表示しません。[指定方法](manual/05-writing-and-structure.md#画像) |
| `![[ノート]]`、`![[ノート#見出し]]` | そのノートの本文をその場に組み込みます（深さ 3 まで。循環は拒否） |
| `[[ノート]]`、`[[ノート\|表示名]]` | 本に含まれるノートならリンク、含まれなければプレーンテキスト |
| `- [ ]` | ☐ / ☑。フォーム部品ではなく文字として描画 |
| `$E = mc^2$`、`$$…$$` | 数式。組版のときに MathML（Temml）へ変換します。閲覧時にスクリプトを読み込むことはないので、PDF でも EPUB でも、オフラインでもそのまま出ます。通貨の `$` は `\$` と書けばそのまま出ます（→ [数式](#数式)） |
| `mermaid` / `dataview` のコードブロック | Obsidian のレンダラで描画し、図として配置 |
| `#tag`、`%%comment%%`、`^block-id` | 削除 |

どの段階も設定タブで個別に無効化できます。またいずれもコードブロックの中には手を出しません。変換は Markdown のソースではなく、文書ツリーに対して行われるからです。

![サンプルの見開きを原寸で。遠雷のルビ、「その手袋は、もう戻らない」の傍点、正立した 10 と 42、連続した空行が開ける紙面の空き、柱とノンブル。](docs/images/spread.png)

### 数式

`$…$` と `$$…$$` は組版のときに MathML へ落とします。Temml が変換するので、数式は文書そのものの一部になり、PDF でも EPUB でも、ネットワークのない Vault でも同じように出ます（閲覧時に MathJax を読みに行く方式は使いません。本はコードを実行しないので、そのスクリプトは書き出しの段階で取り除かれます）。縦組みでは、行の中の数式も別行立ての数式も横組みのまま置かれます。

`$` が数式の始まりと読まれるのは、次の 3 つをすべて満たすときだけです。

- 開きの `$` の直後が空白でない
- 閉じの `$` の直前が空白でない
- 閉じの `$` の直後が数字でない

そのため `価格は $100 から $200 です` や `$1,000 から $2,000` は数式になりません。読まれてしまうのは `価格は$100から$xまで` のように、空白を挟まず `$` が 2 つ並び、2 つ目の直後が数字でないときです。その場合は `\$` と書けば `$` がそのまま出ます（`&dollar;` やコードスパンでも同じです）。数式そのものを止めるなら `vfm: { math: false }` を書きます。

## 段組

同人誌の B6・A5、そして新書は、縦組みでも二段組で組まれることが多い判型です。`novel-2col` テーマがそれを組みます。

```yaml
# vivlio.yaml
theme: novel-2col
size: JIS-B6
charsPerLine: 23   # 1 段の字詰め
linesPerPage: 17   # 1 段の行数
```

**字詰めと行数は 1 段あたりの数です。** 縦組みでは段が上下に並び、どちらの段も紙面の幅いっぱいに行を並べるので、1 ページに載る行数は `linesPerPage` の 2 倍になります。文字は右上から下へ流れ、上の段を左端まで使いきると下の段の右上に続きます。

段数だけを変えたいときは `columns:` を書きます。明示した段数はテーマに関係なく本文へ適用されるので、`novel` の縦組みだけでなく、`manual` の横組みや自作テーマも段組にできます。`columns: 1` なら `novel-2col` も一段へ戻ります。グリッドを持たないテーマでは既存の本文領域を分割するだけで、文字サイズや余白は変えません。

セットアップウィザードのプリセットにも「新書 縦組み二段組」「B6 縦組み二段組」「A5 縦組み二段組」が並びます。判型と段数から本文の文字サイズが逆算されるので、字詰めと行数を書き換えれば紙面はそれに追従します。

扉・copyright page・目次・奥付・表紙は段組にしません。段に割れた奥付は奥付ではないからです。脚注（`gcpm`）はページの地、両方の段にまたがって置かれます。

段組の本文に表がある場合は警告が出ます。狭い段ではセル内の文字が極端に折り返されたり、表が紙面からはみ出したりするためです。警告は書き出しを止めないので、プレビューで確認し、収まらなければその原稿を一段組にしてください。EPUB は段組を解除するため、この警告を出しません。

## テーマを自作する

`theme:` には Vault 内のスタイルシートのパスも書けます。0.17.4以降では `./`・`../` で始めると指定元の YAML（frontmatter ならノート）のフォルダ基準です。本と CSS をまとめて移動・配布する場合はこちらを推奨します。それ以外のパスと全体設定は従来どおり Vault ルート基準です。そのスタイルシートは、同梱テーマを出発点にできます。

```css
/* vivlio.yaml と同じフォルダ内の 装丁/私の本.css */
@import url("vivlio:novel");

:root {
  --vs-novel--boten-font-size: 0.32rem;
  --vs-novel--secondary-ink: #4a4a4a;
}

.callout-warning { border-color: #b00; }
```

```yaml
# vivlio.yaml
theme: ./装丁/私の本.css
```

0.17.3以降では `novel`・`novel-2col`・`essay`・`haiku`・`tanka`・`bunko` の読み込み元の既定グリッドを引き継ぎ、判型に応じて文字サイズを自動計算します。`charsPerLine`・`linesPerPage`・`columns` は省略できます。字数・行数・段数を変える場合は本設定で指定してください。`baseFontSize` を明示した場合はその値が優先されます。[章扉サンプルCSS](sample/novel-title-page.css) を使うと、各章タイトルとその本文開始を、それぞれ独立した左ページにできます。

テーマの選択欄には、このプラグインのために作られた 8 つのテーマ —— `novel`（小説を縦組みで）、`essay`（一般書・エッセイを縦組みで）、`haiku`（句集）、`tanka`（歌集）、`novel-2col`（小説を縦組み二段組で）、`english-novel`（英語小説を欧米のペーパーバック風に）、`manual`（マニュアル・技術書を横組みで）、`paper`（論文・レポートを横組みで）—— に続いて、**Vault 内のすべての `.css` ファイルがそのパスで並びます**。Vault のどこかにスタイルシートを置けばそれだけで候補に出るので、登録の手続きはありません。CC0 の Vivliostyle テーマである `vivlio:base`、`vivlio:bunko`、`vivlio:techbook`、`vivlio:academic` も、本が名指せば解決されますが、選択欄には出しません。このプラグインのノンブルや見出しと突き合わせた確認がまだ済んでいないためです。

### 論文・レポート

Vivlio 0.14.0以降のウィザードで **論文・レポート（A4・横組み）** を選ぶと、`academic` を調整した `paper` テーマを使います。複数原稿を自然につなぎ、章・節・図表を論文全体で自動採番して、目次とID参照へ反映します。要旨・参考文献・付録などはノートの `vivlio-paper-role` で指定し、付録はA、A.1形式にします。

図はキャプションと一緒にページ上部へ浮動配置し、後続の本文で余白を埋めます。長表は表題・列見出しを繰り返してページをまたぎます。[原稿サンプル](sample/paper/index.md) と [設定例](sample/paper/vivlio.yaml) は、手入力の番号や追加CSSを使いません。テーマの選択だけでは判型・組方向は変わりません。複数原稿の処理を有効にするには `theme: paper` を選択してください。CSSのimportだけでは通し採番処理は有効になりません。


これ以外の `@import` は通常の取り込みで、書いたファイルからの相対パスとして Vault から読まれます。各ファイルは一度だけ辿るので、import が輪になっていても問題ありません。使う前に 1 枚のスタイルシートに畳まれるので、プレビューと EPUB がまったく同じ内容を読みます。

CSS の `url(...)` が指すローカルファイルは、その宣言を書いた CSS 自身の場所から解決します。`@import` した CSS の背景画像やフォントも書籍アセットへ書き換え、EPUB に収録します。ただしフォントの収録には **EPUB にフォントを埋め込む**をオンにする必要があります（既定はオフ）。外部 URL、データ URL、フラグメントだけの参照はそのまま残し、ローカルファイルが見つからなければ書き出し前に警告します。

```css
/* 装丁/parts/callouts.css から 装丁/images/paper.png を参照 */
.callout { background-image: url("../images/paper.png"); }
```

書くときに知っておくとよいクラス: `.boten`、`.tcy`、`.callout` と `.callout-<種別>`、`.task-list`、`.vivlio-page-break`、`.vivlio-blank-lines`、`.vivlio-no-indent`、`.vivlio-rendered`、`.copyright-page`、`.copyright-page-content`。

## ビルド

```bash
npm install
npm run build
```

リリースが配るのは `main.js`、`manifest.json`、`styles.css` の 3 つです。ビルド済みの Vivliostyle ビューアと 4 つの CC0 テーマはバンドルに埋め込まれているので、ほかにコピーするものはありません。

```bash
npm test
```

テストは実際の変換パイプライン、設定の各層、ローカルサーバを、Obsidian の外で、アプリ API の小さなスタブに対して動かします。

## リリース

`npm version patch`（または `minor` / `major`）が、新しい番号を `package.json`、`manifest.json`、`versions.json` に一度に書き込みます。そこで作られたタグを push すれば、それがリリースのすべてです。ワークフローがバンドルをビルドし、タグとマニフェストが一致することを確かめ、3 つのファイルを個別のアセットとしてアップロードします。これが Obsidian のインストーラが期待する形です。

```bash
npm version patch
git push --follow-tags
```

## しくみ

```
ノート ──▶ VFM（＋このプラグインのフック）──▶ HTML + 生成された CSS
                                                │
                                    127.0.0.1（トークンで限定）
                                                │
                            ┌───────────────────┴───────────────────┐
                            ▼                                       ▼
                iframe + Vivliostyle ビューア        隠し webview → printToPDF
                        （プレビュー）                 → pdf-lib（しおり、
                                                          メタデータ、ページラベル）
```

Vivliostyle は文書とその素材を XHR で取得するため `file://` が使えません。そこでプラグインは、プレビューか書き出しが開いている間だけ、ビルド結果をループバックで配信します。このサーバは `127.0.0.1` にのみバインドし、すべての URL にセッションごとのトークンを要求し、`Host` ヘッダを検査し、GET と HEAD にしか答えず、CORS ヘッダを返さず、Vault の外のパスを拒否します（明示的に設定されたフォントの場所だけが例外です）。

## ライセンス

AGPL-3.0-or-later。`@vivliostyle/core` と `@vivliostyle/viewer` は AGPL-3.0 で、`main.js` にバンドルされるため、プラグイン全体が AGPL-3.0 になります。[LICENSE](LICENSE) と [NOTICE](NOTICE) を参照してください。VFM は Apache-2.0、テーマは CC0-1.0 です。

フォントは一切同梱しません。あるフォントを PDF や EPUB に埋め込んでよいかをそのライセンスで確認するのは、利用者の責任です。
