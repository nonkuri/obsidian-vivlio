 ...
 * node test/run.mjs test/horizontal-magazine.check.ts
 * Writes review output under output/horizontal-magazine (not the sample).
 */
...
const base = 'horizontal-magazine';
...
async function main() {
  const out = 'output/horizontal-magazine';
  ...
  try {
    ...
    for (const cropMarks of [true, false]) {
      ...
      assert.equal(result.length, 4, 'cover, two-page feature and one-page workshop');
      ...
      for (const p of result) {
        if (p.titleRect) assert.ok(p.titleRect.w / mm > 170, 'article heading spans both columns');
        for (const img of p.images) {
        if (img.src?.includes('plan')) continue;
      ...
      if (cropMarks) {
        await page.pdf({ path: `${out}/horizontal-magazine.pdf`, printBackground: true, preferCSSPageSize: true });
      }
