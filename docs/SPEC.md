...
**本筋の直し方は「刷らない紙面は数えない」だが、まだ組み上がっていない。**
リセットで辻褄を合わせるのではなく、ノンブルを刷らない紙面のカウンタを止めれば、
最初にノンブルを刷る紙面が自然に 1 になり、まえがきから本文まで 1 本で通る。
リセットが要らなくなるので上の制約にも当たらない。名前付きページの
`counter-increment: page 0` がその手段で、**単独では効く**（実測: 表紙・扉・目次の本で
