...
最後の 1 つが手がかりになる。**要素に書いた `counter-reset: page` は、その要素が
2 ページ以上を占めるときだけ、後続のドキュメントに引き継がれる。**

前付けに「まえがき」を置いて測ると、こうなる（表紙・扉・目次のあとに、まえがきでリセット）:

| まえがきの長さ | まえがき | 続く本文 |
|---|---|---|
| 1 ページ | 1 | **5**（リセット前の素のカウンタ） |
| 6 ページ | 1〜6 | **7**（引き継がれる） |

本文の `.vivlio-page-reset` が効いて見えるのは、一章目の要素がふつう何ページも占めるからである
（実測: 羅生門 1・鼻 20・蜘蛛の糸 32 と連番）。1 ページで終わる要素では、リセットはその 1 ページで
終わり、次のドキュメントはリセット前の値に戻る。表紙が 1 ページなのも同じ理由。
**`counter-set` に替えても結果は同じ**（実測）。scoped counter の寿命の問題ではないらしい。

この 1 点が塞がっていることが、次の 2 つを同時に止めている。

- **表紙を数から外せない**（上記）
- **「まえがき」を本文と通しのノンブルにできない。** 和書は前付けにノンブルを刷らず、
  まえがきから本文まで 1 本の算用数字で通すのがふつうで、それには算用数字の起点を
  一章目ではなくまえがきへ移せばよい。**まえがきが 2 ページ以上ならそれで正しく組める**
  （実測: まえがき 1〜6、本文 7〜）。**1 ページのまえがきでは本文が 5 から始まってしまい、
  いまの挙動より悪くなる**ので、この 1 点が解けるまで移さない

なお **`break-before: left` / `right` は効き、入った白丁も数えられる**（下記）。
カウンタそのものを動かす道具が効かないだけで、ページの側を指定する道具は使える。

**【却下】「ノンブルを刷らない紙面は数えない」という直し方は採れない。**
一時そう考えたが、成り立たない。**空白ページはノンブルを刷らないが、ページ数には数える。**
章を左ページから起こすために入る白丁がまさにそれで、「刷らない＝数えない」と決めてしまうと
その 1 枚が勘定から消え、以降のノンブルが 1 つずれる。刷るか刷らないかと、数えるか数えないかは
別の問いである。表紙を数から外したいのは「表紙が本文用紙の一葉ではない」からであって、
「ノンブルを刷らないから」ではない。

なお、名前付きページの `counter-increment: page 0` は**単独では効く**（実測: 表紙・扉・目次の本で
`@page titlepage { counter-increment: page 0 }` を足すと扉が ii から i に落ちた）。
表紙 1 枚だけを外す道具としてはまだ使えるかもしれないが、前付け全体に当てる道具ではない。

ところが**組み合わせると効かなくなる。** 実測:

| 書き方 | 結果 |
|---|---|
| `@page titlepage { counter-increment: page 0 }` だけ | 効く |
| `@page cover, cover-document, titlepage, halftitle, dedication, epigraph, toc, colophon { … }` と 1 つの規則に並べる | **規則ごと落ちる**（可視性の指定も効かない） |
| 名前ごとに規則を分ける | 表紙だけ効き、扉・半扉・献辞は数え続ける |
| 5 つの名前だけの規則に戻す | 扉は止まるが半扉・献辞は数え続け、目次で 1 つ飛ぶ |

一貫した説明が付けられていない。**Vivliostyle の `@page` におけるカウンタ操作の挙動を
先に突き止めること。** それが分かるまで、表紙の件もまえがきの件も直せない。
（別案: 前付けだけ別カウンタで数える、組版後にページ数を測って二度組む）
...
白丁は**ノンブルを刷らないが、ページ数には数える**（数えないと以降が 1 つずれる）。
...
- **表紙が通しページに数えられている**（5.9）。前付けのノンブルが 1 つずつ後ろにずれ、
  PDF のページラベルとも食い違う。**同じ 1 点が「まえがきを本文と通しのノンブルにする」も止めている**
  —— 要素の `counter-reset: page` は、その要素が 2 ページ以上を占めるときだけ後続へ引き継がれるため
  （5.9 に実測表）。カウンタを動かす道具は `counter-reset`・`counter-set`・名前付きページの
  `counter-increment` まで試して、単独では効くものも組み合わせると効かなくなる。
  0.5.0 で目次より前の紙面がノンブルを刷らなくなったので**いま目に見えるずれは「まえがき」以降だけ**
  だが、直ってはいない。なお `break-before: left` / `right` は効き、入った白丁も数えられる（5.11）ので、
  効かないのはカウンタを直接動かす道具に限られる
...
いまのところ `npm test` は 224 件を通す（`convert` / `config` / `server` の 3 ファイル）。
