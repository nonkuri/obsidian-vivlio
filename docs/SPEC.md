# obsidian-vivlio 仕様・実装記録

Obsidian の Markdown を Vivliostyle で日本語・英語組版し、プレビュー付きで PDF / EPUB に出力するプラグイン。

作成日: 2026-09-01 / 調査時点の各パッケージ版数を明記
最終更新: 2026-09-23 — **0.14.0** の実装に合わせ、設定の適用範囲・EPUB フォント同梱条件・検証情報を更新。

この文書は、着手時の技術調査と実装フェーズを履歴として残しつつ、現在の仕様、設計上の決定、
未実装項目を一か所で追跡する。初期案から実装時に変わった点は **【実装時の修正】**、
確定した挙動は **【決定】** と記す。
...
Vault の .md（1枚 / フォルダ / 目次ノート）
 ...
 └────────────────────────────┘
        ▼
 ┌────────────────────────────┐
 │ 3. メモリ上のワークスペース     │  index.html, ch*.html, publication.json,
 │                              │  theme.css とアセット参照を保持し、
 │                              │  Vault の画像はコピーせず配信
 └────────────────────────────┘
        ▼
 ┌────────────────────────────┐
...
| コマンド | 挙動 |
|---|---|
| `Vivlio: プレビューを開く` | 右ペインに `VivlioPreviewView`（ItemView）を開く。アクティブな Markdown は単一ノート、任意の `.yaml` は親フォルダを一冊として選択した設定で組版 |
| `Vivlio: PDF に書き出す` | アクティブな Markdown、または任意の `.yaml` が示す本を、出力設定モーダル → 保存ダイアログ → PDF |
| `Vivlio: EPUB に書き出す` | 同上 → EPUB |
...
| `Vivlio: 本の設定を作成` | 任意の `.yaml` がアクティブなら、その内容を初期値にして同じファイルへ書き戻す。それ以外はウィザードで `vivlio.yaml` を生成（5.4） |
| `Vivlio: このノートに設定を追加` | 入れるキーと値を選び、フラットな `vivlio-*` frontmatter を追加・変更・削除する（5.4） |
...
- リボンアイコン 1 個（プレビュー開閉）
- ファイルエクスプローラのフォルダ / ノート / 任意の `.yaml` の右クリックにメニュー追加。YAML は親フォルダを選択した設定で一冊として PDF / EPUB に書き出す。Markdown / フォルダ起点は同じフォルダの `vivlio.yaml` を既定として読む
- 複数選択は、`files-menu` が渡す項目内に `.yaml` が1つならその設定を対象にする。2つ以上なら、同一フォルダ内でも暗黙に一つを選ばず、設定を一つずつ選ぶよう無効項目を表示する
- 日本語の `doc-colophon` は本文ブロックの最終ページから開始する。縦組み・右綴じでは右ページ、横組み・左綴じでは左ページとし、必要なら直前にノンブルなしの白ページを挿入する。自動生成・ノート指定の両方を対象とし、英語の copyright page には適用しない
...
| 3 | R | **`《《テキスト》》`（カクヨム式傍点）** | 一字ごとに設定した傍点記号（既定はゴマ点）を振った `<ruby class="boten">`（下記）。**ルビより前に置く**（`《《` が `《` にマッチして壊れるため） | ON |
...
傍点記号は `botenMark` で共通に指定する。UI はゴマ点（`﹅`）・白ゴマ点（`﹆`）・黒丸（`●`）・白丸（`○`）・黒三角（`▲`）・白三角（`△`）・二重丸（`◎`）・蛇の目（`◉`）を提示し、任意の記号の直接入力も受け付ける。

#### 傍点は `text-emphasis` ではなくルビで書く【決定】

`text-emphasis: filled sesame` は現代的な綴りだが、**圏点を文字の外側に描くので行ボックスが伸び、
傍点のある行だけ隣の行より広く組まれて縦のグリッドが崩れる。**
ルビはどの行でも同じ帯を確保するため、一字ごとに選択された記号（既定はゴマ点 `﹅`）を振ったルビとして書く ——
...
| 3 | **ノートの frontmatter** | 本全体 | 手書きなら可。GUI ではフラット形式を推奨 | 単独ノート自身、または本の目次ノートからの上書き |
...
botenMark: "﹅"           # 傍点記号。UI の代表例から選択、または任意入力
autoTcy: true
imageWidthUnit: px
...
coverFit: cover
backCover: ""             # 任意の裏表紙画像
backCoverFit: cover       # cover | contain
coverInPdf: true
...
#### 層 3: frontmatter（フラットキーを推奨）

**GUI で編集する設定は `vivlio-` 接頭辞を付けたフラットなキーを使う。** すべて文字列 / 数値 / 真偽値なので、
...
- GUI が提供するのは入れ子が不要なキー。手書きなら `sections` / `labels` / `embedFonts` /
  `colophonExtra` なども、フラットな接頭辞付きキーまたは入れ子の `vivlio:` から読み取れる。
  本全体の設定は通常 `vivlio.yaml` にまとめる。`css` は文字列として指定できる。
...
- 単独ノートではそのノート、フォルダや YAML からの本では目次ノート、リンクからの本では
...
| ステップ | 内容 |
|---|---|
| 1. プリセット | 縦組みの 5 つ（文庫 39x15 / 文庫 40x16 / B6 43x17 / 四六判 44x17 / A5 45x18）／マニュアル・技術書（A5 横組み）／論文・レポート（A4 横組み）／カスタム |
...
| 5. 表紙 | Vault 内の表紙・裏表紙画像／表紙にするノート／各画像の合わせ方／両方をPDFに含めるか |
...
**(2) プロパティの選択と編集 — `Vivlio: このノートに設定を追加`**

開いているノートの**フラットな `vivlio-*` frontmatter** をモーダルで編集する。
**どのキーを入れるか、そのキーに何を入れるかを、キーの説明を読みながら選ぶ。**
既にあるプロパティは現在の値を入れて並び、チェックを外せば削除する。
...
| **既にあるキーは、現在の値を入れて編集可能に見せる** | プロパティパネルはキー名と入力欄しか出さない。`paragraphIndentMode` に何が書けるかも、`manuscript` と `brackets` の違いも、`KEY_DOCS` にはあるがパネルからは読めない。足せるが直せないのでは、書き間違いを直すのにキーを暗記している必要がある |
| **チェックを外すとそのプロパティを削除する** | 一覧がノートの現状を映す以上、チェックは「このノートが持つかどうか」の表示である。足せて外せないのは、同じスイッチの片側だけが効くということ。削除も含めて、書き込む内容はモーダル下部に全部出す |
| **入れ子で書かれているプロパティには触らない** | 1 行の入力欄が持てる値ではない。上書きすれば残りを捨てることになるので、その行だけ編集不可にして理由を書く |
| **値の選択肢（テーマ・判型・字下げ方式…）はウィザードと同じ表から引く**（`keyChoices()`） | 片方にだけ足された選択肢は、もう片方では「無い」ことになる |
| **書き換えるのはフロントマターの該当行だけ**（`frontmatterPatch`） | 他のプラグインのプロパティも、書き手が並べた順序も、閉じ `---` から下も動かさない |
| **各行はウィザードと同じ名前で呼び、キー名（`vivlio-writing-mode`）は添え物にする** | 同じキーを設定する 2 つの入口である。片方で「判型」と呼ぶものを、もう片方で `size` としか書かないのは、書き手に 2 つの語彙を覚えさせる。名前は `KEY_LABELS` から引くので、ウィザードと食い違うことがない |
| **各行の値は、ウィザードと同じ形の入力欄で見せる**（未追加の項目には Vault の既定値を入れておく） | 既定値を埋めて追加するコマンドが、何を埋めるかを見せないのでは、書き手はノートを見に行くまで結果を知らない。欄に打てば、その項目は自動でチェックが付く —— 値を入れた時点で「入れたい」と言っている |
| **モーダルの下に、これから行う追加・変更・削除を全部出す** | チェックの集合より、これから書かれる 5 行のほうが読める。削除できるコマンドが、何を消すかを見せないのは論外でもある。ウィザードの `renderDestination` にあたる |
| **名前と説明で絞り込める** | 一覧は 40 行を超える。ウィザードは 7 ステップに割って長さを解いたが、選択式の一覧に順序はないので、代わりに絞り込みを置く。説明文も検索対象にする（書き手は `botenMark` ではなく「傍点」を探す） |
...
| `highlight` | **`boten`** |
| `botenMark` | **`﹅`（ゴマ点）**。代表的な傍点から選択でき、任意の記号も入力可能 |
...
| `paragraphIndent` | **空（テーマ任せ）。** 字下げの幅を決める。`novel` / `novel-2col` は `1em`、`english-novel` は `1.35em`。`manual` は0.15.0から操作説明に合わせて `0` を既定とする。従来の1字下げは `paragraphIndent: 1em` で復元できる。字下げの対象段落は `paragraphIndentMode` で決める |
...
- **既定のプリセット**: 新規に本の設定を作るときの初期値（文庫・B6・四六判・A5 の縦組み、新書・B6・A5 の二段組、英語小説、マニュアル・技術書、論文・レポート、カスタム）
...
- **フォント**: 本文／見出し／等幅を**インストール済みフォントのドロップダウンから選択**（初期値は 5.10 の OS 横断スタック）。Vault 内フォントフォルダのパス（初期値 `fonts`、自動走査は未実装。`@font-face` の自動生成には `embedFonts` への列挙が必要）。**指定フォントが見つからない場合に警告する ON/OFF**（初期値 ON）。EPUB にフォントを埋め込む ON/OFF（初期値 OFF、ライセンス注意書き付き）
...
- **記法**: 5.3 の各段のトグル。`==ハイライト==` の 4 モード選択（トグルの直下に置く）、傍点記号の代表例選択と任意入力、`dataviewjs / templater` の実行可否（`dynamic` トグルの直下）。`autoTcy` の ON/OFF。行頭全角スペースの削除（#15）だけは字下げ設定の直下、「組版」に置く
...
- **プレビュー**: **自動更新 ON（初期値）／OFF**、デバウンス ms（初期値 600）、全ページ描画 ON/OFF
  - 表示設定: `viewerSpread` = `false`（単ページ、初期値）/ `true`（見開き）/ `auto`、`viewerFitToScreen` = true（初期値）、`viewerZoom` = 1（倍率、0.1〜10）。フィット中も固定倍率を保持する。
  - `rememberViewerSettings` = true（初期値）。ビューアでの表示モード・ズーム／フィット変更を `saveData` で Vault 共通の設定に保存し、プレビュー起動・再組版時に URL の `spread` と `zoom`（フィット時は省略）で復元する。オフの場合は表示中だけ有効で、再組版時に保存済み設定へ戻す。設定タブからの変更は次の起動・再組版時に適用する。複数ペインの変更は最後の操作を既定値とし、他のペインへは次の再組版時に適用する。
  - 同梱ビューアの `history.replaceState` と `hashchange` で URL の表示設定変更を検知し、変更時のみ `postMessage` で親へ通知する。`source`・サーバの `origin`・表示世代 ID・値を検証し、古い表示や不正値を保存しない。記憶する倍率は 10〜1000% に限定する。`localStorage` やビューア内部の DOM・非公開 observable に依存しない。
  - これらはプラグイン専用設定として `data.json` に保存し、本設定のマージ・YAML 書き出し／取り込み・PDF／EPUB 出力には含めない。用紙・余白・文字サイズ・追加 CSS・全ページ描画等のビューア内変更は記憶対象外。PDF 用 URL には表示設定の通知を有効化する ID を付けない。
- **PDF**: タグ付き PDF・栞生成・メタデータ埋め込み・**画像の実効 dpi 警告の閾値**（初期値 300、0 で無効）・表紙・裏表紙を PDF に含める ON/OFF（初期値 ON、両方を一括切り替え）
...
src/
  ...
  config/presets.ts         縦組み各判型 / 二段組 / 英語小説 / マニュアルのプリセット
  ...
  export/epub.ts            JSZip で EPUB 3 梱包
...
**【修正済み】表紙を通しページから除外する。** `vs-counter-doc` は文書数のカウンタであり、
ページ数ではない。表紙の名前付きページに `counter-increment: page 0` を指定する。
扉・目次などの前付けと、改丁で挿入した白紙は数える。ノンブルを刷るかどうかとは別の判断である。

従来の修正が次の文書に引き継がれなかった原因は、同梱 Core が次の文書の開始値を
「前の最終ページを組む前のカウンタ＋1」で求めていたことにある。
最終ページ自身でのリセットや増分0が失われるため、1ページの表紙・章で再現した。
`esbuild/viewer-patch.mjs` は各ページの実際のカウンタ変化量を保存し、文書境界でも引き継ぐ。
再組版に使う開始値と物理的な左右ページの値は変更しない。
組版後の実際のノンブルは PDF のページラベルにも用いる。

**和書向けの既定は `continuous`。** 表紙・表紙裏・裏表紙とその追加白紙を除き、扉・目次・通常の白紙からまえがき・本文・後付けまで
算用数字で通す。まえがきで1に戻さない。既存の `roman-then-arabic` は選択肢として残す。
日本の本でも通しノンブルと別ノンブルの両方があり、どちらか一方だけを出版一般の慣習とはしない。
根拠: [W3C 日本語組版処理の要件 2.6.3](https://www.w3.org/TR/jlreq/#running_heads_and_page_numbers)。
...
- **【決定】表紙と指定した本文ページは塗り足しの外端まで描く。** 表紙画像だけでなく
  `coverPage` の最外側要素（`[role='doc-cover']`）も紙面大にし、背景を含めて裁ち落とす。
  本文では `![[fig.png|bleed]]` を 1 ページの裁ち落とし画像とする。地色や HTML で組んだページは
  最外側の要素に `class="vivlio-bleed"` を付ける:

  ```html
  <div class="vivlio-bleed" style="background: #18202a"></div>
  ```

  通常の画像は従来どおり `applySize` / `imageBackstop` で版面内に収め、`.vivlio-bleed` だけを
  その制限から外す。`@page vivlio-bleed` は余白を 0 にして柱とノンブルを隠す。
  トンボなしでは用紙自体が仕上がり＋塗り足し×2 なので、その 100% を埋める。トンボありでは
  ページ領域が仕上がり寸法のため、要素と表紙を四方へ `bleed` ずつ広げ、左上を `-bleed` へ移して
  仕上がり線の外まで描く。したがってどちらも画像・地色が塗り足しの外端まで届く。
  EPUB は固定した紙を持たないので、強制改ページと固定寸法を解除し、通常の全幅画像として再配置する。
...
- `backCover`（既定は空）で画像を指定し、`backCoverFit: cover | contain`（既定は `cover`）で収め方を選ぶ。パスは `cover` と同じリンク解決規則に従う。ノートのプロパティは `vivlio-back-cover` / `vivlio-back-cover-fit`。
- `back-cover.html` を奥付など全後付けの後ろに追加する。`Chapter.isBackCover` で識別し、`doc-cover` や EPUB の `cover-image` は割り当てない。画像が見つからなければ警告し、白紙を含めて追加しない。
- PDF・プレビューでは `.back-cover-inside` を奇数ページへ送り、その次に `.back-cover` の画像を置く。内側と面合わせの白紙は1〜2ページ。最終ページは物理的な偶数ページ（右綴じでは右、左綴じでは左）とし、ノンブルの開始値に依存させない。
- 名前付きページ `back-cover` は表紙と共通の全面画像・塗り足し処理を使い、`counter-increment: page 0` とする。`:root.vivlio-back-cover` で柱・ノンブルを隠す。裏表紙と追加白紙のPDFページラベルは空とし、目次・ナビゲーション・PDFしおりに載せない。
- `coverInPdf` と書き出しダイアログの「表紙・裏表紙を含める」は両方をまとめて切り替える。個別のオン／オフは設けない。
- EPUBには白紙なしで末尾の画像のみを追加し、表紙の本棚サムネイルは変更しない。`coverInPdf: false` でもEPUBには指定された裏表紙を追加する。
- `test/frontmatter.check.ts` で4テーマ、表紙の有無、本文ページ数、開始ノンブル、奥付、塗り足しを含む32構成の組版とPDFを検証する。
...
- 各 CSS のローカルな `url(...)` は、その CSS 自身の場所から解決してワークスペースのアセット参照へ書き換える。画像は EPUB にも同梱し、`@font-face` のローカルフォントは「EPUB にフォントを埋め込む」が ON の場合だけ同梱する。欠落時は警告する。`data:`、外部 URL、フラグメントだけの参照はそのまま残す
...
#### 段組【決定】: 段の流れはテーマ非依存、グリッド計算はテーマに従う

同人誌の B6・A5、そして新書は縦組みでも二段組が多い。`novel-2col` はそのための調整済みテーマで、
`novel` に二段向けの既定グリッドと見出し寸法を足している。一方、段へ本文を流すこと自体は本の設定
`columns` の責務であり、テーマには限定しない。

**CSS の段は inline 軸に並ぶ。** 横組みでは左右に並んで左段から右段へ、縦組みでは上下に並んで
上段から下段へ続く。したがって同じ `column-count` の規則を両方の書字方向に使え、段の向きを
指示する別の規則は要らない。
...
で、グリッドテーマの `gridFontSize` もこの式で用紙から文字サイズを逆算する。段間はテーマの
...
段数を明示したとき、プラグインが生成する CSS は `:root.vivlio-body body` に `column-count` と
`column-fill: auto` を指定する。したがって `manual` や余白組みの自作テーマでも段組になり、
テーマの既存版面を段へ分けるだけなので、字詰め・行数や文字サイズは勝手に算出しない。
`columns: null` はテーマ任せ、`columns: 1` は二段テーマも明示的に一段へ戻し、2 以上はテーマを問わず
指定段数にする。段間は版面設計の一部なのでテーマに残し、共通処理は上書きしない。

**段組本文に表があれば警告する。** 表には内容から決まる最小幅があり、狭い段へ入れるとセル内が
極端に折り返されるか、段や紙面からはみ出す。フォントと最終的な版面が決まる前に安全な幅を判定する
ことはできないので、表の存在を原稿ノートごとに一度だけ知らせ、書き出し自体は止めない。
プレビューと PDF の書き出し前チェックに出し、段組を解除する EPUB では出さない。
...
- `embedFonts` または CSS から参照するローカルフォントは、設定タブで「EPUB にフォントを埋め込む」を明示的に ON にした場合のみ、
...
sections:
  ...
  colophon: auto           # 奥付
pageNumbering: continuous   # continuous（既定）| roman-then-arabic | none
```
...
**【決定】日本語の奥付は本文ブロックの最終ページにする。** 両面印刷する本文ブロックは
偶数ページで終わるため、縦組み・右綴じでは右ページ、横組み・左綴じでは左ページに置く。
直前の内容が最終ページ側で終わっていれば反対側を白紙にし、次の最終ページ側から奥付を始める。
この規則は `auto` で生成する奥付とノートを指定した奥付の両方に適用する。英語では
copyright page を扉直後に置き、巻末奥付を既定で生成しないため対象外とする。
一般の `startSide` より奥付専用規則を優先する。
...
和書は右綴じで、**奇数ページが左**にくる。章の切り替え・献辞・扉などを
「必ず左ページから」起こすのは製本上の慣習で、そこまで本文が届かなければ**白丁**を 1 枚入れる。
白丁は**ノンブルを刷らないが、ページ数には数える**（数えないと以降が 1 つずれる）。ただし、表紙直後の白紙は表紙裏なので数えない。

`startSide`（`any` / `left` / `right`、既定 `any`）で指定する。CSS では
`break-before: left` / `right` がそのまま「その側のページから始める」であり、
足りなければ組版器が白丁を入れる。当てる先は

```css
h2,
#vivlio-start:not(.cover) > :first-child,
#toc > :first-child,
.halftitle > :first-child { break-before: left; }
```

**部位そのものではなく、その中の最初のブロックに当てる。** 部位は自分のドキュメントの最外要素で、
組版器はそこで既にページを開いているため、そこに置いた break は効かない
（実測: `#vivlio-start { break-before: left }` は扉を右ページに残し、計算値も `auto` だった）。
1 つ内側なら断つべき境目があり、そこで開くページが部位の始まるページになる。

生成・変換のどちらのドキュメントも外側の要素に `#vivlio-start` を持つので、それと、
独自の id / クラスを持つ目次と半扉。**表紙は外す**（先頭の一葉で、その前に断つものがない）。
`h2` も入れるのは、1 ノートで書かれた本の章がそれだから。

白丁を白丁にするのは別の規則で、`startSide` の指定によらず常に出す:

```css
@page :blank { --vs-page--mbox-visibility: hidden; }
```

実測（縦組み・`startSide: left`）: 表紙 1（左）→ 白丁 2（右・柱もノンブルも無し）→ 扉 3（左・iii）
→ 白丁 4 → 目次 5（左・v）→ 白丁 6 → 本文 7（左・1）、章は 7 と 13 でいずれも左。
**扉が iii、目次が v になっていることが、白丁が数えられている証拠**である（ii と iv が白丁）。

#### ノンブル（ページ番号）

`pageNumbering` で選ぶ。和書向けの既定は `continuous`。`vivlio.yaml` / frontmatter の両方から指定できる。

| 値 | 挙動 |
|---|---|
| `roman-then-arabic` | 前付けは `i, ii, iii…`、本文先頭で開始番号にリセット |
| **`continuous`（既定）** | 表紙・表紙裏・裏表紙とその追加白紙を除き、扉・目次・通常の白紙・まえがき・本文・後付けを通しの算用数字で数える |
| `none` | ノンブルを出さない（`--vs-page--mbox-visibility: hidden`）。目次もページ番号を載せない |

- **表紙と表紙裏は数えない。** 表紙直後に面合わせで入る白紙は表紙裏として除外する。それ以外の扉・目次・改丁の白紙は数え、ノンブルを刷らないページも通し番号に含める。
- 前付けのローマ数字は `@page` の名前付きページに `counter(page, lower-roman)` を当て、
  本文先頭の章に **`counter-reset: page <startPage>`** を置く。
...
**【実装済み】`startPage` は整数。** `continuous` では表紙と表紙裏を除いた最初の紙面をその番号から
数え、扉・目次・白丁・まえがき・本文へ続ける。`roman-then-arabic` では本文の一章目をその番号から
数え直す。0以下もカウンターは進めるがノンブルは表示せず、1から表示する。`null` は 1 と同じ。`none` ではノンブル自体を出さないため適用しない。
組版後に各ページの実際の値を読み、PDF の `/PageLabels` の開始番号にも同じ値を使う。
...
#### エディタとプレビューの段落単位の双方向連動（0.18.0）

- プレビューのツールバーに **カーソル連動：オン／オフ** を表示する。表示は現在の状態であり、押すとそのペインの両方向の連動を切り替える。ペインを開いたときはオンとし、この切り替えは永続化しない。
- Markdownの変換前に原稿のファイル・行範囲を保持し、変換後の段落・見出し・箇条書き等へ引き継ぐ。論文の文書結合後に最終HTMLのIDとURLへ対応づける。プレビュー専用の情報とし、PDF・EPUBには付与しない。
- エディタのカーソル移動は150 msのデバウンス後に、その段落の先頭を含むページへ追従する。同じ段落内の移動では繰り返しページ移動しない。
- プレビューの文章を通常クリックすると、原稿の段落先頭行へカーソルとスクロール位置を移動する。既存の原稿タブを再利用し、未表示の原稿は新規タブで開く。画面は分割しない。原稿が閲覧モードなら編集モードへ切り替える。
- リンク・操作部品・修飾キー付きクリック・文字の範囲選択は原稿移動の対象外とする。クリックによる原稿移動を契機にプレビューを段落先頭へ戻す往復を防ぐ。
- 再ビルド中、または原稿のテキストが表示中のプレビューと一致しない間は移動しない。自動更新がオフなら利用者が再ビルドする。古いビューア世代からの同期メッセージを無視する。
- ノート全体・見出し指定の埋め込みでは元ノートの行番号を保持する。複数ページにまたがる段落への追従は先頭ページまでとし、文字単位の対応づけは行わない。対応する段落がない自動生成部分は移動対象外とする。
...
以下は着手時の見積もりを残したもの。Phase 0〜2 と Phase 3 の大部分は完了しており、
現在の未実装範囲は「10. 実装状況」に集約する。
...
当初見積もりは合計 **18〜24 日**（実働）。実装順を判断した根拠として残す。
...
| 28 | 設定の置き場所 | **3 層（設定タブ / `vivlio.yaml` / frontmatter）。frontmatter はフラットな `vivlio-*` キーを推奨し、手書きの入れ子 `vivlio:` も受け付ける**（Obsidian の Properties UI が入れ子 YAML を編集できないため） | 5.4 |
...
| 49 | 傍点記号 | **`botenMark` で本ごとに選ぶ。** ゴマ点・白ゴマ点・黒丸・白丸・三角・二重丸などの代表例を UI に出し、候補にない任意の文字も直接入力できる | 5.3, 5.4 |
| 50 | 複数の本設定 | **`vivlio.yaml` 以外の任意の `.yaml` も一冊の設定として扱う。** 選択した YAML の親フォルダを本文にし、PDF / EPUB 書き出しと設定ウィザードの読み書きに同じファイルを使う。複数選択に YAML が2つ以上ある場合は暗黙に選ばない | 5.2, 5.4 |
| 51 | 日本語の奥付 | **本文ブロックの最終ページに置く。** 縦組み・右綴じでは右ページ、横組み・左綴じでは左ページとし、必要なら直前に白ページを入れる。英語の既定は扉直後の copyright page なので対象外 | 5.11 |
...
**表紙・本文画像・地色の裁ち落とし**、書き出し前チェック、i18n（日英）。
...
| 0.5.0 | **`novel-2col` テーマ**（縦組み二段組）と新書 / B6 / A5 のプリセット、`columns` キー。**目次のページ番号が紙面の刷り方に従うように**（前付けはローマ数字、`none` では番号を載せない）。目次より前の部位を目次に載せない／その紙面にノンブルを刷らない。**トンボなしの塗り足し**。参考文献の番号と本文が詰まる件。ウィザードが `vivlio.yaml` から始まる |
| 0.5.1 | 本文中の外部リンクを自動で脚注にしない（縦組みテーマ）。theme-base の `footnote-external-link.css` が、`novel` の marker 指定と噛み合って `1https://…` になっていた |
| 0.5.2〜0.5.3 | **番号付きリストの番号を縦中横で正立**。`::marker` は行の傍らに置かれて升目に乗らないので、マーカーを止めて生成内容で組み、ピリオドは落とす。目次は巻き込まない |
| 0.6.0 | **`startSide`** —— 章・部を必ず左（右）ページから起こし、足りなければ白丁を入れる。白丁はノンブルも柱も刷らず、ページ数には数える |
| 0.6.1 | **和書向けの通しノンブルを完成。** 既定を `continuous` に変更し、表紙と表紙裏を数えず、扉・目次・通常の白丁・まえがき・本文・後付けを算用数字で通す。1ページの文書で行ったカウンタ操作も次の文書へ引き継ぐ。`startPage` を実装し、0・負数も受け付けて0以下のノンブルは表示しない。PDFページラベルも実際の組版結果に合わせる。従来の `roman-then-arabic` も選択可能 |
...
| 0.9.0 | **傍点記号を設定可能にした。** `botenMark` を設定タブ・YAML・frontmatter の3層で解決し、設定タブとウィザードでは代表的な記号の選択と任意文字の入力に対応。完成サンプルと、ノートごとの CSS を使い分ける手順も公開 |
| 0.10.0 | **別名の YAML を本設定として使い分けられるようにした。** ファイルエクスプローラーで選択した任意の `.yaml` から、その親フォルダを PDF / EPUB に書き出す。複数選択で YAML が2つ以上なら選択を要求する。設定ウィザードも、選択した YAML の内容を読み、同じファイルへ書き戻す。Obsidian 内で YAML / CSS を編集するための Code Space も文書で案内 |
| 0.10.1 | **日本語の奥付を本文ブロックの最終ページへ固定した。** 縦組み・右綴じは右ページ、横組み・左綴じは左ページとし、一般の `startSide` より奥付規則を優先。縦横・ノンブル方式・開始番号・章の長さを組み合わせた実ブラウザー回帰検査を48ケースに拡張。芥川龍之介短編集サンプルも、別名 YAML による A5二段組・四六判・文庫判の作り分けを示す内容へ更新 |
| 0.10.2 | **自作テーマのローカル CSS アセットを収録するようにした。** `@import` した各 CSS 自身の場所を基準に `url(...)` を解決し、背景画像やフォントをプレビュー用アセットへ書き換えて EPUB にも同梱する。欠落ファイルを警告し、外部 URL・データ URL・フラグメントだけの参照は維持する |
| 0.11.0 | **ノートのプロパティをコマンドから編集できるようにした。** 「このノートに設定を追加」の各行をウィザードと同じ名前・同じ説明・同じ選択肢で並べ（`KEY_LABELS` / `keyChoices()`）、既にあるプロパティは現在の値を入れて編集可能にし、チェックを外せば削除する。名前と説明で絞り込め、これから行う追加・変更・削除はモーダル下部に全部出る。書き換えるのはフロントマターの該当行だけ（`frontmatterPatch`）。**`manual` テーマに 1 字分の字下げ**を入れた —— theme-base の `0` を継いだままでは、#15 が原稿の全角スペースを消す以上、横組みの本から字下げが完全に消えていた |
| 0.11.1 | **数式を組めるようにした。** VFM の既定の `mathjax` は LaTeX を本文に残して CDN の MathJax を閲覧時に読み込む方式で、その `<script>` は sanitize が外す（→ 5.12）ため、紙面には `\(x\)` の生テキストだけが残っていた。`mathRenderer` を `mathml` に固定し、Temml がビルド時に MathML へ落とす。あわせて、連続空行の数え直しを原稿由来でない位置のブロックで止め（数式の次の段落が十数行下に落ちていた）、MathML を含む EPUB 文書に `properties="mathml"` を付けた |
| 0.12.0 | **ビューアの表示設定をプラグイン側で保存・復元できるようにした。** 単ページ・見開き・自動、画面フィット、固定ズーム（10〜1000%）を設定可能。ビューアでの変更を記憶する機能は初期値オンで、Vault 共通の設定として再組版・再起動後も復元する。PDF・EPUB の組版設定とは分離し、設定マニュアル・日英 README と回帰テストを追加・更新した |

| 0.12.1 | **novel テーマの奥付で発行所と連絡先・WEBサイトのまとまりを改善した。** 連絡先・WEBサイトの手前の余白をなくし、後続の印刷情報との間に余白を設けた。novel-2col にも適用される |

### マニュアル・操作ガイド（0.15.0）

- 本文は字下げなしを既定とし、見出し・手順・注意・ヒント・コード・キー表記のスタイルを整備。マニュアルのプリセットは一段組を明示する。
- `build/manual.ts` は `theme: manual` の本文にあるキャプション付き図表を原稿順に通し採番する。図と表は別のカウンター。前付け・後付けを採番せず、章・見出し・spineは保持する。
- `data-ref="fig"` / `data-ref="tbl"` で指定した同一原稿・別原稿への参照を文字として確定し、PDFとEPUBで同じラベルを使う。通常のリンクの表示名は変更しない。
- キャプション付き表の8行以下（見出しを含む）は一緒に配置し、長表では表題と列見出しを反復する。継続ページは「続き」を表示する。長い手順・注意書き・コードは分割可能。図はキャプションと一緒に掲載位置へ置き、ページフロートしない。
- スタイルだけを `vivlio:manual` からimportする自作テーマには、採番処理は適用しない。章・節番号は自動付与しない。
- [サンプル](../sample/manual/index.md)は4原稿・模式図2点・設定表・36行の長表を含む。A5は16ページ、A4は12ページを自動組版検査し、PDF全ページを目視確認。EPUBは図・参照・表末尾を構造検査。Obsidian実機と外部EPUBリーダーの表示は検証範囲外。

詳細は[テーマのマニュアル](../manual/06-custom-theme.md#マニュアル操作ガイドを組む)と[利用・検証方法](../sample/manual-README.md)を参照。

### 論文・レポート（0.14.0）

`paper` テーマとA4横組みのプリセットを追加した。上流の `academic` の書式を土台に、Vivlioの扉・目次・柱・ノンブルに対応する。

- 本文原稿を一つのHTMLに結合し、ファイル境界では強制改ページしない。章・節・図表は原稿順に自動採番し、目次とID参照にも反映する。同名IDは原稿別に区別する。
- ノート専用の `vivlio-paper-role` で `body`（既定）、`abstract`、`references`、`appendix`、`unnumbered` を指定する。要旨・文献は通常の章番号から除外し、付録はA、A.1形式とする。
- 図はキャプションと一体でページ上部へ浮動配置し、図の順序を保つ。参考文献・付録の開始前に未配置の図を出し切る。長表は通常フローで分割し、表題・列見出し・継続表示を繰り返す。
- 一段組を通常ブロックフローとして扱い、長表の後の本文が版面外へ流れる問題を防ぐ。参考文献URLの脚注への重複出力を抑止する。
- サンプルは原稿7ファイル、図5点、表6点、数式、脚注、48行の長表、合成データと再現コードを含む。手入力番号・追加CSSを使わず、実機PDF全15ページを確認済み。

詳細は[テーマのマニュアル](../manual/06-custom-theme.md#paperで複数原稿の論文を組む)と[検証記録](../sample/paper/reproduce/verification.md)を参照。
...
- **上流テーマそのものの選択欄への公開。** `bunko` / `techbook` / `academic` / `base` は埋め込み・名前の解決を維持する。選択欄には動作確認済みの調整版 `novel` / `novel-2col` / `english-novel` / `manual` / `paper` を表示する。
...
- **段組中の全段抜き。** 表や図を著者が明示的に `column-span: all` 相当へできる記法を検討する。
  表を自動で全段抜きにすると、表の多い原稿では段組が何度も中断され、ページ分割と余白が大きく変わる。
  自動適用はせず、原稿ごとの判断で指定する別機能にする
### 検証の範囲
...
| 生成 CSS | テーマの `@import`、書字方向・判型・傍点記号の変数、表紙と `.vivlio-bleed` の named page・版面制限解除・塗り足し方向への張り出し、日本語の奥付が縦組みでは右・横組みでは左の最終ページになること |
...
| ノンブルと目次 | 既定が `continuous` であること、`startPage` の正数・0・負数を設定として受け付けること、前付けの行がローマ数字で刷られること、その指定が入れ子の本文に漏れないこと、`none` では目次も番号を載せないこと |
| 綴じ | `startSide` が章と部に効くこと、表紙は外れること、白丁からノンブルと柱が落ちること |
| 用紙 | トンボなしの塗り足しで用紙が仕上がり＋塗り足し×2 に育つこと、トンボありでは紙に触らないこと。どちらでも表紙・本文画像・地色が塗り足しの外端まで届き、裁ち落とし画像の実効 dpi がその幅で計算されること |
| テーマの取り決め | 上流の自動脚注を縦組みテーマが打ち消していること（上流の規則が消えたら気づけるよう、上流側も確認する）、リストの番号が生成内容で組まれ目次を巻き込まないこと |
| EPUB | 梱包した CSS が紙面用の指定を落としていること、JSZip が読める zip を作ること |
| 設定 3 層 | 優先順位、フラット `vivlio-*` キーの読み取り、型強制、未知キー警告、`auto` 誤用の検出、選択した別名 YAML の読み込みとウィザードによる同じファイルへの書き戻し |
| 章順 | ファイル名の自然順、目次ノートの登場順、`vivlio-order` の差し込み |
| ローカルサーバ | トークン不一致 404 / Host 検証 / 405 / パス脱出の拒否 / CORS ヘッダ無し / 停止後に応答しないこと。位置復元では CFI を epage より優先し、URL パラメータを壊す文字を符号化すること |

2026-09-23 に `npm test` の成功を確認した。`convert` / `config` / `server` の **361 チェック**に加え、`viewer` の表示設定・保存・復元チェックを実行する（計 4 ファイル）。

ページングは `test/pagination.check.ts` で実ブラウザも確認する。縦組み / 横組み、
`continuous` / `roman-then-arabic`、`startPage` の正数・0・負数、`startSide` の有無、
...
→ 7. の受け入れ条件は通っている。Vivlio は Obsidian のコミュニティプラグイン一覧へ登録済みで、
この仕様書が対象とするリポジトリのバージョンは **0.15.0**。
