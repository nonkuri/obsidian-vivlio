...
| 17 | R | **`［＃改ページ］`（青空文庫式の強制改ページ）** | 記号を消し、**次のブロックに `.vivlio-page-break` を付ける**（下記） | ON |
...
- **組版**: 既定テーマ（同梱の 5 種と Vault 内の .css、初期値 `novel`）／既定用紙サイズ／既定書字方向／**既定の脚注モード（初期値 `gcpm` = ページ下）**／**字下げの幅と、字下げする段落の決め方（初期値 `auto` = 原稿に従う）**／追加 CSS のパス（Vault 内）
...
- **記法**: 5.3 の各段のトグル。`==ハイライト==` の 4 モード選択。`autoTcy` の ON/OFF
...
| `titlePage` | `title` / `subtitle` / `author` / `publisher` を組んだ扉（下記） |
...
| `title` | 前半 | 書名 |
| `author` | 項目行 | 著者 |
| `translator` | 項目行 | 訳者。あれば |
| `date` | 項目行 | 発行日。**縦組みでは漢数字**（下記） |
| `version` | 項目行 | 版 |
| `publisher` | 項目行 | 発行所。あれば |
| `printer` | 項目行 | 印刷。あれば |
| `contact` | 項目行 | 連絡先（住所・メール）。あれば |
| `website` | 項目行 | WEB サイト。あれば |
| `colophonExtra` | 項目行の末尾 | **著者が決める任意の項目**（下記） |
...
`author` / `publisher` は `<div class="imprint">` にまとめて出す。
...
- 章題は `h1 { string-set: vivlio-chapter content(); }` から取る。
  h2 も足すと、節のある章では**最後に組まれた節の見出し**が柱に出てしまう。
  目次が h1 を章・h2 を節として並べる（`tocDepth` 既定 2）のと同じ見方を柱にも当てる
- **見出しのない章のために、ドキュメントに自分の名前を名乗らせる。**
  見出しから取るだけでは、地の文で始まるノートや、唯一の見出しが書名と同じで落とされた章
  （`dropBookTitleHeadingPlugin`）で文字列が空になり、**左ページの柱だけが消える。**
  プラグインは**見出しを持たない本文ドキュメントに限って**最外の `<section>` に
...
| 45 | 強制改ページ | **`［＃改ページ］`**（青空文庫式）。改ページ用の空要素は置かず、**直後のブロックにクラスを付ける** —— 高さ 0 のブロックはページを消費しないので、そこに改ページを持たせると組版が終わらない | 5.3 #17 |
