...
| 0.6.0 | **`startSide`** —— 章・部を必ず左（右）ページから起こし、足りなければ白丁を入れる。白丁はノンブルも柱も刷らず、通常はページ数に数える（表紙裏だけは除外） |

### 今回の修正（未リリース）

- 表紙をページ数から除外し、1ページの章のカウンタ操作も次章に引き継ぐ。
- 和書向けの既定を `continuous` に変更。扉・目次・白紙・まえがき・本文を通しで数える。
- `startPage` を実装。通し番号では表紙・表紙裏の次、別ノンブルでは本文の開始番号になる。0以下は数えるが表示しない。
- PDF ページラベルを実際の組版結果に合わせる。
