...
# 組版
theme: novel              # novel | <vault内のcssパス>（bunko / techbook / academic / base も解決はする）
...
| ステップ | 内容 |
|---|---|
| 1. プリセット | 文庫本（縦組み）／Web 小説の書籍化／カスタム（技術同人誌・論文はテーマともども保留 → 12「未実装」） |
...
**残る課題**: 上のやり方だと、`100%` のとき箱が絵より大きくなる（絵は箱の中で天地中央に置かれ、
...
- **`novel` 以外の同梱テーマ。** `bunko` / `techbook` / `academic` / `base` は埋め込みも名前の解決も
  生きているが、プラグインの版面・ノンブル・見出しアキは `novel` に対してだけ合わせてあるので、
  選択肢には出さない（`SELECTABLE_THEMES`）。同じ理由で技術同人誌 / 論文のプリセットも外してある
- **フォントフォルダの走査。** 設定タブの「フォントフォルダ」（初期値 `.obsidian/fonts`）は
  `@font-face` を自動生成すると説明しているが、`fontFolder` を読むコードは設定タブ以外に無い。
  実際に `@font-face` を作るのは `vivlio.yaml` の `embedFonts:` に明示列挙した分だけ
  （`build/fonts.ts`）。着手するときに決めること:
  - 指定フォルダの `.woff2` / `.otf` / `.ttf` を拾って `@font-face` を組み立てる
  - **既定値を Vault 内の普通のフォルダに変える。** `.obsidian/` 配下はドットフォルダなので
    Vault の索引に載らず、`vault.getFiles()` では見つからない
