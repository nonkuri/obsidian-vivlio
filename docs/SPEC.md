...
| 生成 CSS | テーマの `@import`、書字方向・判型・傍点スタイルの変数、表紙と `.vivlio-bleed` の named page・版面制限解除・塗り足し方向への張り出し |
...
いまのところ `npm test` は 234 件を通す（`convert` / `config` / `server` の 3 ファイル）。

ページングは `test/pagination.check.ts` で実ブラウザも確認する。`continuous` / `roman-then-arabic`、
`startPage` の正数・0・負数、`startSide` の有無、1ページ / 複数ページの章を組み合わせ、表紙と表紙裏の
除外、通常の白丁の加算、0以下のノンブル非表示、PDFページラベルとの一致を検証する。
