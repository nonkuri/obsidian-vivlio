...
最終更新: 2026-09-07 — **0.10.1** の実装・配布状況に合わせて更新。
...
- 各 CSS のローカルな `url(...)` は、その CSS 自身の場所から解決してワークスペースのアセット参照へ書き換える。画像や `@font-face` のフォントは EPUB にも同梱し、欠落時は警告する。`data:`、外部 URL、フラグメントだけの参照はそのまま残す
...
いまのところ `npm test` は **301 件**を通す（`convert` / `config` / `server` の 3 ファイル）。
