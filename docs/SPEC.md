...
| `paragraphIndent` | **空（テーマ任せ）。** 字下げの**幅**だけを決める。まったく下げないなら `0`。同梱テーマは全て自前で 1 字分を持つ（`novel` と、それを読む `novel-2col` が `1em`、`english-novel` が `1.35em`、`manual` が `1em`）—— #15 が原稿の全角スペースを消す以上、幅を持たないテーマでは字下げが完全に消えるので、同梱テーマが theme-base の `0` を継ぐことは許さない（`manual` がそうなっていた） |
...
この仕様書が対象とするリポジトリのバージョンは **0.14.0**。
